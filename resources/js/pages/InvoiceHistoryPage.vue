...
const invoiceData = ref({ invoiceSummary: [], invoices: [], statuses: ['Todos'], meta: {} });
const isPageChanging = ref(false);
let requestSeq = 0;
const summary = computed(() => invoiceData.value.invoiceSummary ?? []);
const dynamicFilters = computed(() => invoiceData.value.filters ?? {});
const statusFilters = computed(() => dynamicFilters.value.statuses?.length ? dynamicFilters.value.statuses : [{ label: 'Todos', value: 'all' }]);
const yearFilters = computed(() => dynamicFilters.value.years ?? []);
const dateRange = computed(() => dynamicFilters.value.dateRange ?? { min: '', max: '' });
const invoices = computed(() => invoiceData.value.invoices ?? []);
const meta = computed(() => invoiceData.value.meta ?? {});
const pageCount = computed(() => Math.max(1, Number(meta.value.last_page) || 1));
const paginatedInvoices = computed(() => invoices.value);
const isRefreshing = computed(() => invoiceStore.state.refreshing);

const invoiceParams = (page = currentPage.value) => ({
    page,
    per_page: perPage,
    query: query.value,
    ...
    from: dateFrom.value,
    to: dateTo.value,
    include_filters: invoiceData.value.filters?.years?.length ? 0 : 1,
});

const loadInvoices = async (page = currentPage.value) => {
    const seq = ++requestSeq;
    const params = invoiceParams(page);
    const cached = invoiceStore.getCached(params);
    isPageChanging.value = true;
    currentPage.value = page;
    if (cached) invoiceData.value = cached;
    const data = cached && !invoiceStore.isFresh(params)
        ? await invoiceStore.refreshInBackground(params)
        : await invoiceStore.fetchPage(params);
    if (seq !== requestSeq) return;
    if (data) {
        const includesFilters = data.meta?.includes_filters !== false;
        invoiceData.value = {
            ...data,
            filters: includesFilters && data.filters && Object.keys(data.filters).length ? data.filters : invoiceData.value.filters,
            statuses: includesFilters && data.statuses?.length ? data.statuses : invoiceData.value.statuses,
        };
    }
    currentPage.value = Number(data?.meta?.current_page ?? data?.meta?.page ?? page) || page;
    if (!selectedYear.value && invoiceData.value.filters?.years?.[0]?.value) selectedYear.value = invoiceData.value.filters.years[0].value;
    void invoiceStore.prefetchPage({ ...invoiceParams(currentPage.value + 1), page: currentPage.value + 1 });
    isPageChanging.value = false;
};

watch([query, selectedStatus, selectedYear, dateFrom, dateTo], () => {
    void loadInvoices(1);
});

const goToPage = (page) => {
    void loadInvoices(Math.min(Math.max(1, page), pageCount.value));
};
            ...
            <main class="invoice-main shell-content">
                ...
                <section class="invoice-table liquid-panel portal-enter portal-enter-delay-2">
                    ...
                    <div class="invoice-filters">
                        <label><span class="material-symbols-outlined">search</span><input v-model="query" type="search" placeholder="Buscar factura, cliente, vendedor o estado"></label>
                        <select v-model="selectedStatus"><option v-for="status in statusFilters" :key="status.value" :value="status.value">{{ status.label }}{{ Number.isFinite(status.count) ? ` (${status.count})` : '' }}</option></select>
                        <select v-model="selectedYear"><option v-for="year in yearFilters" :key="year.value" :value="year.value">{{ year.label }}{{ Number.isFinite(year.count) ? ` (${year.count})` : '' }}</option></select>
                        <input v-model="dateFrom" type="date" aria-label="Desde" :min="dateRange.min || undefined" :max="dateRange.max || undefined">
                        <input v-model="dateTo" type="date" aria-label="Hasta" :min="dateRange.min || undefined" :max="dateRange.max || undefined">
                    </div>
                    <table :class="{ 'is-changing': isPageChanging }">
                        <thead>
                            <tr>
                                <th>Factura</th>
                                <th>Fecha</th>
                        ...
                        <tbody>
                            <tr v-for="invoice in paginatedInvoices" :key="invoice.number">
                                <td><strong>{{ invoice.number }}</strong></td>
                                <td>{{ invoice.date }}</td>
                                <td>{{ invoice.due }}</td>
                                <td><StatusBadge :status="invoice.status" :tone="invoice.tone" size="sm" /></td>
                    ...
                    <PaginationControl :current-page="currentPage" :last-page="pageCount" :total="Number(meta.total) || 0" :per-page="Number(meta.per_page) || perPage" :disabled="isPageChanging" @page-change="goToPage" />
                </section>
            </main>
        </template>
        <template #mobile>
            <main class="invoice-mobile">
                ...
                <article v-for="invoice in paginatedInvoices" :key="invoice.number" class="invoice-card liquid-panel portal-enter">
                    <div class="card-top"><strong>{{ invoice.number }}</strong><StatusBadge :status="invoice.status" :tone="invoice.tone" size="sm" /></div>
                ...
                <PaginationControl :current-page="currentPage" :last-page="pageCount" :total="Number(meta.total) || 0" :per-page="Number(meta.per_page) || perPage" :disabled="isPageChanging" compact @page-change="goToPage" />
            </main>
        </template>
    </AppShell>
</template>
...
.invoice-main.shell-content{width:100%!important;max-width:none!important;margin:0!important;padding:18px!important}.invoice-hero,.summary-grid,.invoice-table{width:100%!important}.summary-grid{grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:clamp(16px,1.4vw,24px)!important}.invoice-table table{width:100%!important}.liquid-panel,.summary-card{box-shadow:0 28px 90px rgba(0,0,0,.3),0 0 42px rgba(56,189,248,.055),inset 0 1px 0 rgba(255,255,255,.08)}.invoice-filters,.mobile-invoice-filters{position:relative;z-index:1;display:grid;grid-template-columns:minmax(220px,1fr) 180px 150px 150px 150px;gap:10px;padding:16px;border-bottom:1px solid rgba(255,255,255,.08)}.invoice-filters label{display:flex;align-items:center;gap:8px}.invoice-filters input,.invoice-filters select,.mobile-invoice-filters input,.mobile-invoice-filters select{min-width:0;border:1px solid rgba(125,211,252,.18);border-radius:12px;background:rgba(10,16,30,.55);color:#e0e8f0;padding:10px 12px;outline:0}.mobile-invoice-filters{grid-template-columns:1fr;margin:14px 0;padding:14px;border-radius:18px}.mobile-invoice-filters button{display:flex;align-items:center;justify-content:center;gap:8px;border:1px solid rgba(125,211,252,.28);border-radius:12px;background:rgba(125,211,252,.12);color:#9ee8ff;padding:10px 12px;font-weight:900}.invoice-pagination{position:relative;z-index:1;display:flex;align-items:center;justify-content:center;gap:12px;padding:16px}.invoice-pagination button{border:1px solid rgba(125,211,252,.24);border-radius:999px;background:rgba(125,211,252,.1);color:#9ee8ff;padding:9px 14px;font-weight:900}.invoice-pagination button:disabled{cursor:not-allowed;opacity:.45}.invoice-pagination span{color:#a9bac8;font-size:12px;font-weight:900}.empty-state{position:relative;z-index:1;padding:18px;text-align:center;color:#a9bac8}@media(max-width:1100px){.summary-grid{grid-template-columns:1fr!important}.invoice-filters{grid-template-columns:1fr 1fr}}
</style>
